...
function App() {
  ...
  const addCardHandler = (_) => {
    setCards([...cards, cardGenerator()]);
  };
  ...
  return (
    <ThemeProvider theme={Theme}>
      <div className="App">
        {cards.length > 0 && (
          <Button color="mango" length={cards.length} onClick={toogleShowCard}>
            Toogle show/hide
          </Button>
        )}
        <p>
          <button className="button button-green" onClick={addCardHandler}>
            Add card
          </button>
        </p>
        {cardsMarkup}
